...
###############################################################################
    def load_results(self, table):
        ...
        for row in table[1:]:
            ...
            race_times = {}
            for i in headers:
                race_times[headers[i]] = float(times[i])
            race_dict[horse_name] = race_times
        return race_dict
...
###############################################################################
    ...
    def horse_personal_best(self):
        ...
        for horse in self.race_dict:
            fastest_time = self.horse_fastest_time(horse)
            fastest_race = self.horse_fastest_race(horse)
...
###############################################################################
    ...
    def get_average_time(self):
        ...
        '''
        average = {}
        for races in self.race_dict.items():
            total_time = 0
            count = 0
            for time in races.values():
                total_time += time
                count += 1
            average[horse] = total_time / count
        return average
